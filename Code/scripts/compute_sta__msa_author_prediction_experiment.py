...
experiment_dataframe = pd.DataFrame.from_records([
    {
        'book_name': job_info['book'],
        'prompt': prompt,
        'predicted_author': predicted_author,
        'ground_truth_author': ground_truth_author,
    }
    for book_path in list_dir(experiment_path, return_paths=True, default_value=[])
    for job_info in [read_from_json_file(f'{book_path}/job.json')]
    for prompt, predicted_author, ground_truth_author in zip(
        read_from_json_file(f'{book_path}/prompts.json'),
        read_from_json_file(f'{book_path}/predicted_outputs.json'),
        read_from_json_file(f'{book_path}/ground_truth_outputs.json'),
    )
])
...
sta_per_author_dataframe = pd.DataFrame([
    {
        'book_name': book_name,
        'accuracy': compute_accuracy(book_name=book_name),
    }
