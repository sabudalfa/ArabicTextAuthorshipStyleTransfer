...
EXPERIMENT_PATH = '/hdd2/style_transfer/jrcai_authorship/output/experiments/31 - AceGPT-13B/'
...
run_llm_jobs_in_parallel([
    LLMTrainingJob(
        llm_trainer=LLMTrainer(
            ...
            eval_samples=get_samples(
                llm_name=llm_loader.name,
                split='eval',
                author=author,
                use_fine_tuning_prompt=True,
            ),
            peft_config=LoRAConfigRepository.jais_v1() if llm_loader.name == 'jais-family-30b-16k-chat' else LoRAConfigRepository.llama_3(),
            epochs_count=10,
            learning_rate=2.5e-4,
            train_batch_size=1,
            eval_batch_size=1,
            output_dir=f'{EXPERIMENT_PATH}/{author}/fine-tuned-models/{llm_loader.name}-for-author-style-transfer/',
        ),
        gpus_count=4 if llm_loader.name == 'jais-family-30b-16k-chat' else 2,
    )
    for llm_loader in llm_loaders
    for author in authors + ['all_authors']
])
...
run_llm_jobs_in_parallel([
    ...
    for job in [
        TextGenerationJob(
            llm_loader=llm_loader,
            experiment_path=f'{EXPERIMENT_PATH}/{author}/outputs/{llm_loader.name}/zero-shot/',
            gpus_count=2 if llm_loader.name == 'jais-family-30b-16k-chat' else 1,
